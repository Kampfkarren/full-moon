...
fn test_position_min_max() {
	...
	impl<'ast> Visitor<'ast> for TestVisitor {
        fn visit_table_constructor(&mut self, constructor: &ast::TableConstructor<'ast>) {
			self.0 = true;
			assert_eq!(MIN_MAX_CODE.bytes().nth(constructor.start_position().unwrap().bytes()), Some(b'{'));
			assert_eq!(MIN_MAX_CODE.bytes().nth(constructor.end_position().unwrap().bytes() - 1), Some(b'}'));
		}
	}
